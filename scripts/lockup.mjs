 ...
 */
export function generate(suffix, { variant = 'mono', accent = null } = {}) {
  const v = SPEC.variants[variant];
  if (!v) throw new Error(`unknown variant ${variant}, expected ${Object.keys(SPEC.variants).join(' | ')}`);

  const { pens, scale, ink } = layout(suffix);
  const { hexagon, logotype } = artwork();
  const vbW = Math.round(ink.maxX * 1000) / 1000;
  const H = SPEC.logoSpace.height;

  const dot = pens[0];
  const dotPath = glyphPath('.', dot.x, scale);
  const letters = pens.slice(1).map((p) => glyphPath(p.ch, p.x, scale)).join('');
  ...
  const header =
    `<!-- NIMIQ.${suffix} lockup. GENERATED by \`nq lockup ${suffix}\`; regenerate rather than hand-edit.\n` +
  ...
  return (
    ...
    defs +
    `<path class="nq-hex" fill="${mono ? v.hexagon : `url(#${gid})`}" d="${hexagon}"/>` +
    `<path class="nq-wordmark" fill="${v.ink}" d="${logotype}"/>` +
...
export function inspectSource(src, file = '') {
  ...
  // Outlined form.
  if (!src.includes(LOGOTYPE_MARKER)) return found;
...
export async function cmdLockup(args, flags = {}) {
  ...
  if (!sub || sub.startsWith('-')) {
    console.log(`nq lockup <suffix> [--variant light|dark|mono] [--accent #RRGGBB]
...
assets/lockup/spec.json for where each one comes from.`);
  ...
  process.stdout.write(generate(suffix, { variant, accent }));
  return 0;
}
