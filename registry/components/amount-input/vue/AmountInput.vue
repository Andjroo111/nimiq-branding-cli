<template>
    <div class="amount-input" :class="{ 'has-value': valueInLuna > 0, focussed: isFocussed }">
        <form class="label-input" ref="fullWidth$" @submit.prevent>
...
const props = withDefaults(defineProps<{
    /** Amount in the currency's smallest unit (Luna for NIM): displayed as value / 10^decimals. */
    value?: number,
    /** Maximum font size in rem; shrinks when the value would overflow the available width. */
    maxFontSize?: number,
    placeholder?: string,
    /** Removes the input's inset border (nq-input "vanishing" style). */
    vanishing?: boolean,
    decimals?: number,
    /** Currency code, e.g. 'nim', 'tnim', 'btc', 'mbtc', 'tbtc', 'usdc.e'. */
    currency?: string,
}>(), {
    value: undefined,
    maxFontSize: 8,
...
const emit = defineEmits<{
    /** Emitted with the entered amount in the smallest unit (upstream Vue 2 v-model event). */
