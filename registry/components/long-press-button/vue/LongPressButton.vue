...
// Vue 3 port of @nimiq/vue-components LongPressButton.vue.
// The 'Keep pressing…' prompt is the en-US i18n source string (I18nMixin stubbed).
import { onBeforeUnmount, onMounted, ref, useSlots, watch } from 'vue';

const props = withDefaults(defineProps<{
    /** How long the button must be held before 'long-press' fires, in milliseconds. */
    duration?: number,
    /**
     * A supported Nimiq button color, see https://github.com/nimiq/nimiq-style/blob/master/src/buttons.css, or
...
const emit = defineEmits<{
    /** Emitted once the button has been held for the full duration. */
    (e: 'long-press'): void,
}>();
...
watch(isPressed, (pressed) => {
    if (pressed) {
        ...
        longPressTimeout = window.setTimeout(() => emit('long-press'), props.duration);
    } else {
...
<style scoped>
    ...
    /* Vue 3 rename: upstream's Vue 2 '.transition-indicator-enter' is '-enter-from' in Vue 3. */
