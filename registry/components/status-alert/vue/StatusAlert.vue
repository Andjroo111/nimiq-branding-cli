<template>
  <div class="nq-status-alert" :class="variantClass">
    <p class="alert-title">{{ title || defaultLabel }}</p>
    <p><slot>{{ message }}</slot></p>
  </div>
</template>

<script setup>
// StatusAlert — Vue 3 port of the real Nimiq alert/callout pattern.
// Upstream source: nimiq-ui packages/nimiq-vitepress-theme/src/assets/github-callouts.css
// (.custom-block.github-alert). No upstream Vue component exists (callouts are
// markdown-rendered in the docs), so this is a faithful wrapper around the
// upstream CSS, with `type` mapped to the upstream variant classes.
import { computed } from 'vue';

const TYPE_MAP = {
  info: { cls: 'note', label: 'Note' },
  success: { cls: 'tip', label: 'Tip' },
  warning: { cls: 'warning', label: 'Warning' },
  error: { cls: 'caution', label: 'Caution' },
  important: { cls: 'important', label: 'Important' },
};

const props = defineProps({
  /** Alert severity: maps to upstream note/tip/warning/caution/important. */
  type: {
    type: String,
    default: 'info',
    validator: (v) => Object.keys(TYPE_MAP).includes(v),
  },
  /** Title line; defaults to the upstream label for the type. CSS uppercases it. */
  title: { type: String, default: '' },
  /** Body text; the default slot overrides it. */
  message: { type: String, default: '' },
});

const variantClass = computed(() => (TYPE_MAP[props.type] ?? TYPE_MAP.info).cls);
const defaultLabel = computed(() => (TYPE_MAP[props.type] ?? TYPE_MAP.info).label);
</script>

<style scoped>
/* Upstream github-callouts.css, namespaced + rem->px at the 16px docs root
   (see registry html/status-alert.css for the porting notes). */
.nq-status-alert {
...
.nq-status-alert p :deep(a) {
  color: currentColor;
  text-decoration: underline;
}

.nq-status-alert p :deep(pre),
.nq-status-alert p :deep(code) {
  color: currentColor;
